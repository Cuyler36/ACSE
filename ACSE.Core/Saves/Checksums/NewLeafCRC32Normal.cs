...
namespace ACSE.Core.Saves.Checksums
{
    public sealed class NewLeafCRC32Normal : IChecksum<uint>
    {
        ...
        /// <param name="initialValue">The initial value of the CRC32. Defaults to 0.</param>
        /// <returns>The calculated CRC32 checksum.</returns>
        public uint Calculate(in IList<byte> data, uint initialValue = 0)
        {
            var size = data.Count;
        ...
        public bool Verify(in IList<byte> data, uint currentCrc32, uint initialCrc32 = 0) =>
            Calculate(data, initialCrc32) == currentCrc32;

        // Non-interface methods

        /// <summary>
        /// Gets a type 2 CRC32 checksum for an IList of bytes, given an offset and size.
        /// </summary>
        /// <param name="data">IList of bytes that contains the desired data to be checksummed.</param>
        ...
        public uint GetChecksum(in IList<byte> data, int offset, int length) =>
            Calculate(data.Skip(offset).Take(length).ToArray());
    }
}
