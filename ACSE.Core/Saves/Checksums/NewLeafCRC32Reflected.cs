...
{
    public sealed class NewLeafCRC32Reflected : IChecksum<uint>
    {
        ...
        /// <param name="initialCrc32">The initial CRC32 value</param>
        /// <returns>The calculated CRC32 checksum.</returns>
        public uint Calculate(in IList<byte> data, uint initialCrc32 = 0xFFFFFFFF)
        {
            var size = data.Count;
            var crc = initialCrc32;
            var p = 0;
            while (size-- != 0)
        ...
        public bool Verify(in IList<byte> data, uint currentCrc32, uint initialCrc32 = 0xFFFFFFFF) =>
